<div class="relative z-[20] bg-darkColor">
    ...
    >
        ...
        <ul class="flex w-[60%] items-center justify-center gap-x-5">
            @foreach(\App\Services\Nav::headerPages() as $page)
                @if($page['name'] == 'Shop')
                    ...
                    />
                @else
                    <x-layout.header.home.nav-item :title="$page['title']" url="{{route($page['route'])}}" />
        ...
        <div class="flex w-[20%] items-center justify-end gap-x-5">
            ...
            <x-icons.cart-icon x-data @click="$dispatch('toggle-cart')"
                               class="h-7 w-7 cursor-pointer hover:scale-110 transition-transform duration-300" />
        </div>
    </nav>
</div>
{{-- Mega Menu --}}
